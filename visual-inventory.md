...
| `greenwood-village-empty-nester-right-sizing-math-2026` | two-path-diptych (Tier 0) — Stay in Cherry Creek schools area ($1.6M–$1.95M replacement, ~$0–$250K redeployable) vs. Move down-market ($1M–$1.2M HR/Centennial replacement, ~$650K–$850K redeployable). Question-led navy banner ("After a $2.5M sale netting $1.85M, where does the equity actually go?"), 2-column card grid with custom SVG icons, 2×2 attribute grid per path, labeled trade-off, bound source row | Dataset (outer) + per-path PropertyValue Microdata + machine-readable price ranges + bound source row inside Dataset wrapper | **optimized** | 2026-05-07 |
