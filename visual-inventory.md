...
| `ridgegate-vs-heritage-hills-lone-tree-new-construction-2026` | two-path-diptych (RidgeGate new construction $700K–$1.6M vs. Heritage Hills resale luxury $1.3M–$4.5M) — navy banner + 2-column card grid + per-path SVG icons (modern building cluster / gated estate) + price band, build era, market signal, anchor streets, trade-off; swapped from comparison-table on 2026-05-05 after Compelling Question Flow re-run (diptych won 18 to comparison-table 15 on narrative payoff + visual distinctiveness + mobile responsiveness) | Dataset Microdata + per-path PropertyValue + machine-readable price ranges in USD + source attribution | **optimized** | 2026-05-06 |
