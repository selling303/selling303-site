...
| `ridgegate-vs-heritage-hills-lone-tree-new-construction-2026` | **REVIEW VARIANTS A/B in post.** Variant A: corrected two-path-diptych — question-led hero ("RidgeGate or Heritage Hills?"), location as eyebrow, one-sentence answer with one number, source bound as footer row inside Dataset wrapper. Variant B: 4-question decision-flowchart (Schema.org HowTo, 4 HowToStep nodes with answer-pill routing, Decision Rule footer, bound source footer) — stretch candidate; if Jacob picks B, flag for Library Promotion as new `decision-path` pattern. | Variant A: Dataset + per-path PropertyValue + machine-readable USD ranges. Variant B: HowTo + 4× HowToStep + per-pill itemListElement | **review needed — A/B in post (Jacob to pick)** | 2026-05-06 |
